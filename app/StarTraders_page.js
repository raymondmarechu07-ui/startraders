...
const LOADING_STEPS = [
  { label: 'Verifying your account', threshold: 0 },
  { label: 'Connecting your trading session', threshold: 25 },
  { label: 'Securing your session', threshold: 55 },
  { label: 'Preparing your dashboard', threshold: 82 },
];

const SCAN_MESSAGES = [
  'Verifying your account...',
  'Connecting your trading session...',
...
export default function HomePage() {
  ...
  // Smooth loading sequence — about 5.2 seconds.
  useEffect(() => {
  ...
  useEffect(() => {
    const idx = progress < 25 ? 0 : progress < 55 ? 1 : progress < 82 ? 2 : 3;
    setScanMsgIndex(idx);
  }, [progress]);
  ...
  const activeStep = progress < 25 ? 0 : progress < 55 ? 1 : progress < 82 ? 2 : 3;
    ...
    return (
      <div id="loading-screen">
        <div className="load-card">
          ...
          <div className="welcome-p">We're preparing your trading workspace.</div>

          <div className="loading-steps">
            {LOADING_STEPS.map((step, index) => {
              const complete = progress >= [25, 55, 82, 100][index];
              const current = !complete && activeStep === index;
              return (
                <div key={step.label} className={complete ? 'loading-step complete' : current ? 'loading-step current' : 'loading-step'}>
                  <span className="step-indicator">{complete ? '✓' : ''}</span>
                  <span className="step-label">{step.label}</span>
                  <span className="step-status">{complete ? 'Complete' : current ? 'In progress' : 'Pending'}</span>
                </div>
              );
            })}
          </div>

          <div className="loading-almost">
            <strong>{progress >= 100 ? 'Ready.' : 'Almost there...'}</strong>
            <span>Great trades start with great preparation.</span>
          </div>
    ...
    <div id="homepage" className="visible">
      <nav className="home-nav">
        ...
        <div className="home-nav-links">
          <span>Markets</span><span>Features</span><span>Copy Trading</span><span>Academy</span>
        </div>
        <div className="home-nav-actions">
          <span className="language-pill">◎ EN</span>
          <span className="support-pill">◉ Support</span>
          <button className="login-btn" onClick={login}>Login Now →</button>
        </div>
      </nav>
      <div className="home-market-strip" aria-hidden="true">
        <span>‹</span><span>BULL MARKET ---</span><span>BEAR MARKET ---</span><span>VOL 10 ---</span><span>VOL 25 ---</span><span>VOL 50 ---</span><span>VOL 75 ---</span><span>VOL 100 ---</span><span>VOL 25 ---</span><span>›</span>
      </div>
      ...
      <section className="hero">
        ...
        <div className="hero-cta-row">
          <button className="btn-cta-lg" onClick={login}>Start Trading Now →</button>
          <button className="btn-cta-secondary" onClick={login}>Create Account</button>
        </div>
