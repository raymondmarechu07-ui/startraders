...
export default function UtilityBar() {
  ...
  useEffect(() => {
    const saved = typeof window !== 'undefined' ? window.localStorage.getItem('st-theme') : null;
    if (saved === 'light') {
      setTheme('light');
    ...
    <>
      <div className="utility-bar">
        <div className="utility-left">
          <button className="utility-menu-button" aria-label="Open menu" onClick={() => setSideMenuOpen(true)}>
      ...
      {sideMenuOpen && (
        <div className="side-menu-backdrop" onClick={() => setSideMenuOpen(false)}>
          <div className="side-menu-panel" onClick={(e) => e.stopPropagation()}>
            <div className="side-menu-header">
              <button className="side-menu-close" aria-label="Close menu" onClick={() => setSideMenuOpen(false)}>×</button>
              <span className="side-menu-title">Menu</span>
              <span className="side-menu-lang"><span aria-hidden="true">🇬🇧</span> EN</span>
            </div>
            <div className="side-menu-body">
              <button type="button" className="side-menu-row" onClick={() => { setSideMenuOpen(false); setContactOpen(true); }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16.5c0 .6-.4 1-1 1-8.3 0-15-6.7-15-15 0-.6.4-1 1-1h3.5c.5 0 .9.4 1 .9.1 1.2.4 2.4.8 3.5.2.4.1.9-.3 1.2l-1.6 1.3c1.4 2.9 3.8 5.3 6.7 6.7l1.3-1.6c.3-.4.8-.5 1.2-.3 1.1.4 2.3.7 3.5.8.5.1.9.5.9 1v3.5z" /></svg>
                <span>Contact us</span>
              </button>
              <button type="button" className="side-menu-row" onClick={toggleTheme}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36A5.4 5.4 0 0112 3z" /></svg>
                <span style={{ flex: 1 }}>Dark theme</span>
                <span className={theme === 'dark' ? 'side-menu-toggle on' : 'side-menu-toggle'}><span className="side-menu-toggle-dot" /></span>
              </button>
              {isLoggedIn && (
                <button type="button" className="side-menu-row" onClick={() => { setSideMenuOpen(false); logout(); }}>
