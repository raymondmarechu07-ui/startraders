...
// Heights are in px (not %) because the flex row that holds them has
// no fixed height of its own — a percentage would resolve to 0.
function buildCandles(count, offset) {
  return Array.from({ length: count }).map((_, i) => {
    const seed = (i * 37 + offset * 53) % 97;
    const height = 26 + (seed % 74); // 26 - 100 px
    const up = seed % 2 === 0;
    const wick = 10 + (seed % 14); // px
    return { height, up, wick, key: `${offset}-${i}` };
  });
}

const CANDLES = buildCandles(30, 0);
  ...
  return (
    <div id="loading-screen">
        ...
        <div className="candle-track">
          {[...CANDLES, ...CANDLES, ...CANDLES].map((c, i) => (
            <span
              ...
              style={{ '--h': `${c.height}px`, '--w': `${c.wick}px` }}
            />
          ))}
        </div>
      ...
      <div className="load-ticker">
        ...
        <div className="load-ticker-viewport">
          <div className="load-ticker-track">
            {Array.from({ length: 6 }).flatMap(() => TICKER_ITEMS).map((item, i) => (
              <div className="load-ticker-item" key={`${item.name}-${i}`}>
                <b>{item.name}</b>
